...
function(prepare_prebuilt_files filepath)
    ...
    if(NOT EXISTS ${filepath})
        message(STATUS "webview_cef: ${filepath} not found")
        set(need_download TRUE)
    else()
        if(NOT EXISTS ${filepath}/version.txt)
            message(STATUS "webview_cef: ${filepath}/version.txt not found")
            set(need_download TRUE)
        else()
            file(READ ${filepath}/version.txt local_version)
            if(local_version STREQUAL cef_prebuilt_version)
                if(has_release_lib)
                    message(STATUS "webview_cef: CEF prebuilts are up to date")
                else()
                    message(STATUS "webview_cef: version file is present but CEF libraries are missing in ${filepath}")
                    set(need_download TRUE)
                endif()
            else()
                set(need_download TRUE)
    ...
    if(need_download)
        message(STATUS "webview_cef: updating CEF prebuilts in ${filepath}")
