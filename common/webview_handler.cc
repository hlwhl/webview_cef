...
namespace
{
    ...
    // Normalize frame identifier across different CEF builds where
    // CefFrame::GetIdentifier() can return either an integer id or a CefString.
#if __linux__
    inline int64_t FrameIdToInt64(const CefString &id)
    {
        // Non-throwing conversion using strtoll; returns 0 on failure.
...
#else
    inline int64_t FrameIdToInt64(const int64_t id)
    {
        return id;
    }
#endif
...
void WebviewHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
{
    ...
    if (!browser->IsPopup())
    {
        ...
        // Track open browser count for safe shutdown.
        webview_cef::NotifyBrowserCreated();
    }
}
...
void WebviewHandler::createBrowser(std::string url, std::function<void(int)> callback)
{
    ...
    {
        auto self = this;
        CefPostTask(TID_UI, new LambdaTask([self, url, callback]()
                                           { self->createBrowser(url, callback); }));
        return;
    }
#endif
...
{
    // Cookie ops belong on the IO thread. Bail if wrong thread.
...
void WebviewHandler::sendJavaScriptChannelCallBack(const bool error, const std::string result, const std::string callbackId, const int browserId, const std::string frameId)
{
    ...
    if (bit != browser_map_.end())
    {
        ...
        // Return types for frame->GetIdentifier() differ across CEF builds.
        // On Linux in our prebuilt it returns a CefString-like type; convert to int.
        // Convert current frame id to int64 in a portable way.
#if __linux__
        int64_t currentFrameId = FrameIdToInt64(frame->GetIdentifier());
#else
        int64_t currentFrameId = FrameIdToInt64(frame->GetIdentifier());
#endif
        bool identifierMatch = currentFrameId == frameIdInt;
        if (identifierMatch)
        {
